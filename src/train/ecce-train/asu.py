...
def model_run(net,waveshape,frequency,temperature):
    ...
    fft_data=scipy.signal.resample(waveshape,24,axis=1)
    print(fft_data.shape)
    avoidZero=1e-3
    Flux =np.abs(fft_data*1024/24+avoidZero) 
...
def one_sample_predict(material,waveshape,frequency,temperature):
    ...
    net.load_state_dict(torch.load(f"models/asu/{MAT2FILENAME[material]}"))
    net.eval()
...
if __name__ == "__main__":
    material="3C90"
    waveshape=0.3*np.sin(np.linspace(-np.pi/2,2*np.pi-np.pi/2,1024)).reshape(1,1024)
    waveshape=np.
    vstack((waveshape,waveshape))
    temperature=np.array([[25],[90]])
    frequency=np.array([[200e3],[200e3]])
    pred=one_sample_predict(material,waveshape,frequency,temperature)
    print(pred/1e3,"kW/m^3")
    # plt.show()
    mdl = AsuModel("models/asu/Model3C90.sd",material="3C90")
